...
layer/temp data, plus `sam-p1s-realistic-nonlinear-layers` and `vic-h2c-realistic-full-completion`
— each recorded live by polling Bambuddy's own status endpoint directly, so their
progress/layer/remaining-time values are genuinely verbatim, including the real, non-linear
relationship between progress% and layer count caused by the printer's own start
routine/calibration counting toward progress% before per-layer printing begins (see
`replayRuns.js`'s own comments). `replayRuns.test.js` sanity-checks the run data itself (ordering,
